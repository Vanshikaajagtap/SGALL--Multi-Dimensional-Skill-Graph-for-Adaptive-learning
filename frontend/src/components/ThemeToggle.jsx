...
export default function ThemeToggle() {
  const [dark, setDark] = useState(() => {
    const stored = localStorage.getItem('skillweave_theme');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('skillweave_theme', dark ? 'dark' : 'light');
  }, [dark]);
  ...
  return (
    <button
      id="theme-toggle"
      onClick={() => setDark((d) => !d)}
      className="relative w-10 h-10 flex items-center justify-center rounded-xl transition-all duration-200 hover:scale-110"
      style={{
